    ...
    <>
      ...
      <div className="lg:hidden mb-6">
        <button
          ...
          className="inline-flex items-center gap-2 text-xs font-mono tracking-wider text-[#1d6ea8] hover:text-[#0f1712] transition-colors"
      ...
      {mounted && createPortal(
        <>
          ...
          <div
            className={`lg:hidden fixed top-0 left-0 z-[100] h-full w-[280px] max-w-[85vw] bg-white border-r border-black/10 shadow-2xl flex flex-col transition-transform duration-300 ease-out ${isOpen ? "translate-x-0" : "-translate-x-full"
              }`}
          >
            {/* Drawer Header */}
            <div className="flex items-center justify-between px-5 py-5 border-b border-black/10">
              <span className="text-xs font-mono uppercase tracking-wider text-[#1d6ea8] font-bold">
                Contents
              </span>
              <button
                onClick={() => setIsOpen(false)}
                className="text-[#526054] hover:text-[#0f1712] transition-colors text-base"
                  ...
                  <li key={h.id}>
                    <button
                      ...
                      className={`
                        ...
                        ${activeId === h.id
                          ? "text-[#1d6ea8] font-bold"
                          : "text-[#526054] hover:text-[#0f1712]"
                        }
                      `}
                    >
                      {h.text}
      ...
      <nav className="hidden lg:block sticky top-10 self-start max-h-[calc(100vh-4rem)] overflow-y-auto pr-4">
        <p className="text-[10px] font-mono uppercase tracking-[0.15em] text-[#526054]/70 mb-4">
          Contents
        </p>
